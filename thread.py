...
def show_thread(id):
    sql = """SELECT H.title, H.op_content, 
             (SELECT U.username FROM users U WHERE U.id = H.user_id),
             R.id, R.content, R.user_id, (SELECT U.username FROM users U WHERE U.id = R.user_id), 
             R.sent_at, H.id, H.user_id FROM threads H LEFT JOIN replies R ON H.id = R.thread_id WHERE H.id=:id"""
